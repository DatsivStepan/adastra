...
$corePath = HTTP_SERVER.'view/image/magicslideshow-opencart-module';
?>
        ...
        <table class="params" cellspacing="0">
            <tr class="last">
                ...
                <td width="70%">
                    ...
                    <?php if ($this->config->get('magicslideshow_status')) { ?>
        ...
        foreach ($parameters as $name => $s) {
			...
			if ($s['id'] == 'thumb-max-height' && !isset($s['value']) && in_array($profile,array('featured','bestseller','latest','special'))) {
			    $s['default'] = '100';
                        ...
                        if($s['id'] == 'selectors-size') continue;
                            ...
                            switch($s["type"]) {
                                case "array":
                                        ...
                                        foreach($s["values"] as $p) {
                                            ...
                                            $rButtons[strtolower($p)] = '<label><input type="radio" value="'.$p.'"'. ($s["value"]==$p?"checked=\"checked\"":"").' name="'.$profile.'_'.$s['id'].'" id="magicslideshowsettings'. ucwords(strtolower($name)).$p.'">';
                                            $pName = ucwords($p);
                                            if(strtolower($p) == "yes")
                                ...
                                default:
                                    ...
                                    if (!isset($s["value"])) $s["value"] = $s["default"];
                                    $html .= '<input '.$width.' type="text" name="'.$profile.'_'.$s['id'].'" id="magicslideshowsettings'. ucwords(strtolower($name)).'" value="'.$s["value"].'" />';
                                    break;
                            }
...
<?php
function trial_bage() {
if (!preg_match('/DEMO/is',@file_get_contents(DIR_CATALOG.'view/javascript/magicslideshow.js'))) return;
echo '
