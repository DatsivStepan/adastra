			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				foreach (json_decode($cart['option']) as $product_option_id => $value) {
                    ...
                    if ($option_query->num_rows) {
                        if ($option_query->row['name'] == 'Высота' || $option_query->row['name'] == 'Ширина') {
                            $order_width  = $option_query->row['name'] == 'Ширина' && $value > 0 && $order_width  == "+"  ? $value : $order_width;
                            $order_height = $option_query->row['name'] == 'Высота' && $value > 0 && $order_height == "+"  ? $value : $order_height;
                        }
                    }
                }
				...
				foreach (json_decode($cart['option']) as $product_option_id => $value) {
					$option_query = $this->db->query("SELECT po.product_option_id, po.option_id, od.name, o.type FROM " . DB_PREFIX . "product_option po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE po.product_option_id = '" . (int)$product_option_id . "' AND po.product_id = '" . (int)$cart['product_id'] . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'");
					if ($option_query->num_rows) {
						if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'image' || $option_query->row['type'] == 'custom') {
							...
							if ($option_value_query->num_rows) {
								if ($option_value_query->row['price_prefix'] == '+') {
                                    ...
                                    }elseif ($option_value_query->row['option_name'] == "Фактура"){
                                        $option_price += $order_width*$order_height/10000 * $option_value_query->row['price'];
                                    }elseif ($option_value_query->row['option_name'] == "Рама"){
                                        $option_price += (($order_width+$order_height)*2 + 8*$option_value_query->row['field1'])/100* $option_value_query->row['price'];
                                    }else{
                                        $option_price += $option_value_query->row['price'];
                                    }
								    /*CUSTOM CODE PRICE LOGIC END*/
							...
							foreach ($value as $product_option_value_id) {
								$option_value_query = $this->db->query("SELECT pov.option_value_id, ovd.name, pov.quantity, pov.subtract, pov.price, pov.price_prefix, pov.points, pov.points_prefix, pov.weight, pov.weight_prefix, od.`name` AS option_name  FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) LEFT JOIN oc_option_description AS od ON od.option_id = ov.option_id WHERE pov.product_option_value_id = '" . (int)$product_option_value_id . "' AND pov.product_option_id = '" . (int)$product_option_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
                                /*CUSTOM CODE PRICE LOGIC START*/
								$i++;
								if ($option_value_query->num_rows) {
									if ($option_value_query->row['price_prefix'] == '+') {
                                        if ($option_value_query->row['option_name'] == "Подрамник"){
                                            $option_price += ($order_width+$order_height)*2/100 * $option_value_query->row['price'];
                                        }else{
                                            $option_price += $option_value_query->row['price'];
                                        }
                                        /*CUSTOM CODE PRICE LOGIC END*/
